...
public class SimulatorResource {
    ...
    private JsonObject setDataBaseMode(JsonObject dataBaseObj) throws Exception {
        ...
        if (!dataBaseObj.containsKey("date-ini")) {
            ...
            entity = JSON.createObjectBuilder().add("error", "No sim-config -> database -> database-user provided")
                    .build();
        } else if (!dataBaseObj.containsKey("database-password")) {
            entity = JSON.createObjectBuilder().add("error", "No sim-config -> database -> database-password provided")
                    .build();
        } else {
            databaseMode = true;
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @RequestBody(name = "sim-config", required = true, content = @Content(mediaType = "application/json", schema = @Schema(type = SchemaType.STRING, example = "{\"sim-config\" : {\"num-orders\": 10,\"pizza-status\":\"ORDERED\"}}")))
        ...
        try {
            if (jsonObject == null) {
                ...
                if (entity != null)
                    ...
                    resp = createOrders(dataBaseObj.getString("date-format"), dataBaseObj.getString("date-ini"),
